...
-(void)confirmselect{
    ...
    self.EOSbalance = 0;
    self.usdtbalance = 0;
    if (_wallet.coinType == MIS) {
        [self.amountView.balancelb setText:[NSString stringWithFormat:@"%@：%.6f MIS", NSLocalizedString(@"余额", nil),self.MISbalance]];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"TransactionVC"];
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [MobClick endLogPageView:@"TransactionVC"];
...
- (void)viewDidLoad {
    ...
    if (self.wallet.coinType == MIS || self.wallet.coinType == EOS) {
...
-(void)nextAction{
    ...
    if (_wallet.coinType == MIS) {
        [_deview.amountlb setText:[NSString stringWithFormat:@"%.4f MIS",amount.doubleValue]];
        [_deview.infolb setText: [NSString stringWithFormat:@"MIS%@",NSLocalizedString(@"转账", nil)]];
    }else if (_wallet.coinType == EOS){
    ...
    [CreateAll CreateETHTransactionFromWallet:walletETH ToAddress:address Value:value callback:^(Transaction *transactionresult) {
        ...
        if (transaction) {
            ...
            [CreateAll GetBalanceETHForWallet:walletETH callback:^(BigNumber *balance) {
                ...
                [CreateAll ETHTransaction:transaction Wallet:walletETH GasPrice:gasbignumber GasLimit:weakSelf.GasLimit callback:^(HashPromise *promise) {
                    ...
                    if (promise.error) {
                        ...
                        [weakSelf.view showMsg:NSLocalizedString(@"交易已广播", nil)];
                        //把记录传给后台
                        [NetManager AddTradeWithuserName:[CreateAll GetCurrentUserName] TradeAmount:weakSelf.amountView.amountTextField.text.doubleValue Poundage:gasbignumber.unsignedIntegerValue*weakSelf.GasLimit.unsignedIntegerValue CoinCode:@"ETH" From:weakSelf.addressView.fromAddressTextField.text To:weakSelf.addressView.toAddressTextField.text CompletionHandler:^(id responseObj, NSError *error) {
                            if (!error) {
                                if (![[NSString stringWithFormat:@"%@",responseObj[@"resultCode"]] isEqualToString:@"20000"]) {
                                    //[weakSelf.view showAlert:[NSString stringWithFormat:@"Error:%ld",error.code] DetailMsg:responseObj[@"resultMsg"]];
                                    return ;
...
-(void)loadDataToView{
    ...
    if (self.wallet.coinType == BTC || self.wallet.coinType == BTC_TESTNET) {
        ...
        [self.amountView.balancelb setText:[NSString stringWithFormat:@"%@：%.4f MIS", NSLocalizedString(@"余额", nil),self.MISbalance]];
...
-(void)scanBtnAction{
    ...
    [WBVC setGetQRCodeResult:^(NSString *string) {
        ...
        }else if(weakSelf.wallet.coinType == MIS){
            if ([string containsString:@"MIS"]) {
                string = [string componentsSeparatedByString:@"@MIS@"].lastObject;
                [weakSelf.addressView.toAddressTextField setText:string];
            }else{
                [weakSelf.view showMsg:[NSString stringWithFormat:@"%@ MIS %@", NSLocalizedString(@"不是有效的", nil),NSLocalizedString(@"地址", nil)]];
            }
...
-(void)getBinToJson:(NSString *)datastring Handler:(void(^)(id response))handler{
    ...
    if (self.wallet.coinType == MIS) {
        [[HTTPRequestManager shareManager] post:eos_abi_bin_to_json paramters:params success:^(BOOL isSuccess, id responseObject) {
        ...
        [[HTTPRequestManager shareEosManager] post:eos_abi_bin_to_json paramters:params success:^(BOOL isSuccess, id responseObject) {
            NSLog(@"%@",responseObject);
            if (isSuccess) {
                handler(responseObject);
            }
...
- (void)getJson:(NSDictionary *)dic Binargs:(void(^)(id response))handler {
    if (self.wallet.coinType == MIS) {
        [[HTTPRequestManager shareManager] post:eos_abi_json_to_bin paramters:dic success:^(BOOL isSuccess, id responseObject) {
        ...
        [[HTTPRequestManager shareEosManager] post:eos_abi_json_to_bin paramters:dic success:^(BOOL isSuccess, id responseObject) {
...
- (void)getInfoSuccess:(void(^)(id response))handler {
    if (self.wallet.coinType == MIS) {
        ...
        MJWeakSelf
        [[HTTPRequestManager shareEosManager] post:eos_get_info paramters:nil success:^(BOOL isSuccess, id responseObject) {
...
- (void)getRequiredPublicKeyRequestOperationSuccess:(void(^)(id response))handler {
    ...
    if (self.wallet.coinType == MIS) {
        ...
        MJWeakSelf
        [[HTTPRequestManager shareEosManager] post:eos_get_required_keys paramters:[self getPramatersForRequiredKeys] success:^(BOOL isSuccess, id responseObject) {
...
//    NSLog(@"getPramatersForRequiredKeys ============= %@",[self getPramatersForSign]);
    ...
    if (self.wallet.coinType == MIS) {
        [[HTTPRequestManager shareManager] post:eos_push_transaction paramters:pushDic success:^(BOOL isSuccess, id responseObject) {
            ...
            if (isSuccess) {
                [NetManager AddTradeWithuserName:[CreateAll GetCurrentUserName] TradeAmount:weakSelf.amountView.amountTextField.text.doubleValue Poundage:0 CoinCode:@"MIS" From:weakSelf.addressView.fromAddressTextField.text To:weakSelf.addressView.toAddressTextField.text CompletionHandler:^(id responseObj, NSError *error) {
                    if (!error) {
                        if (![[NSString stringWithFormat:@"%@",responseObj[@"resultCode"]] isEqualToString:@"20000"]) {
                            // [weakSelf.view showAlert:[NSString stringWithFormat:@"Error:%ld",error.code] DetailMsg:responseObj[@"resultMsg"]];
                            return ;
        ...
        [[HTTPRequestManager shareEosManager] post:eos_push_transaction paramters:pushDic success:^(BOOL isSuccess, id responseObject) {
            NSLog(@"tran response %@",responseObject);
            if (isSuccess) {
                [NetManager AddTradeWithuserName:[CreateAll GetCurrentUserName] TradeAmount:weakSelf.amountView.amountTextField.text.doubleValue Poundage:0 CoinCode:@"EOS" From:weakSelf.addressView.fromAddressTextField.text To:weakSelf.addressView.toAddressTextField.text CompletionHandler:^(id responseObj, NSError *error) {
                    if (!error) {
                        if (![[NSString stringWithFormat:@"%@",responseObj[@"resultCode"]] isEqualToString:@"20000"]) {
                            // [weakSelf.view showAlert:[NSString stringWithFormat:@"Error:%ld",error.code] DetailMsg:responseObj[@"resultMsg"]];
                            return ;
...
- (NSDictionary *)getAbiJsonToBinParamters {
    ...
    if (self.wallet.coinType == MIS) {
        [args setObject:[NSString stringWithFormat:@"%.4f MIS", amount.doubleValue] forKey:@"quantity"];
    }else{
