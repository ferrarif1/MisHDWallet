...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"EOSWalletDetailVC"];
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [MobClick endLogPageView:@"EOSWalletDetailVC"];
...
- (void)viewDidLoad {
    ...
    if (self.wallet.coinType == EOS) {
        ...
        [self.scrollView addHeaderRefresh:^{
            ...
            [[HTTPRequestManager shareEosManager] post:eos_get_actions paramters:@{@"account_name":namex ,@"pos":@0,@"offset":@1000} success:^(BOOL isSuccess, id responseObject) {
...
-(void)GetBaseInfo{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *symbol = nil;
        switch (weakSelf.wallet.coinType) {
            case BTC:
                symbol = @"BTC/USDT";
                break;
            case ETH:
        ...
        [NetManager GetMarketBaseInfoWithMySymbol:symbol Lang:[currentLanguage isEqualToString:@"english"]?USD_TYPE:CHY_TYPE completionHandler:^(id responseObj, NSError *error) {
            if (!error) {
                ...
                if (![weakSelf.baseInfoModel isEqual:[NSNull null]] && weakSelf.wallet.coinType == EOS) {
                    dispatch_async_on_main_queue(^{
                        [weakSelf.detailview.infotextView setText:weakSelf.baseInfoModel.summary == nil?@"":weakSelf.baseInfoModel.summary];
                    });
                }
...
-(WalletDetailView *)detailview{
    if (!_detailview) {
        ...
        if (_wallet.coinType == MIS || _wallet.coinType == EOS){
...
-(EOSDetailBtnsView *)eosdetailbtnsview{
    if (!_eosdetailbtnsview) {
        ...
        [_eosdetailbtnsview.marketBtn setTitle:NSLocalizedString(@"行情走势", nil) forState:UIControlStateNormal];
...
- (void)getAccountSuccess:(void(^)(id response))handler {
    NSString *username = [_wallet.walletName componentsSeparatedByString:@"_"].lastObject;
    if ([CreateAll ifMisWalletRegistered] == NO) {
        [self.view showMsg:NSLocalizedString(@"钱包帐号注册中", nil)];
        return;
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.wallet.coinType == MIS){
        ...
        if ([from isEqualToString:name] && [to isEqualToString:name]) {
            ...
            cell.amountlb.text = [NSString stringWithFormat:@"0.0000 MIS"];
            [cell.amountlb setTextColor:[UIColor textGrayColor]];
            cell.addresslb.text = to;
        }
        ...
        if ([@"transfer" containsString:VALIDATE_STRING(model.action_trace.act.name)]){
            if ([from isEqualToString:name] && [to isEqualToString:name]) {
                cell.amountlb.text = [NSString stringWithFormat:@"+%@ EOS", amount];
                [cell.amountlb setTextColor:[UIColor textGrayColor]];
                cell.addresslb.text = from;
            }else if([from isEqualToString:name]){
                cell.amountlb.text = [NSString stringWithFormat:@"-%@ EOS", amount];
