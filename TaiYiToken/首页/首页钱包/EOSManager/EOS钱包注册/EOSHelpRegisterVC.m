...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"EOSHelpRegisterVC"];
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [MobClick endLogPageView:@"EOSHelpRegisterVC"];
}
- (void)popAction{
...
- (void)viewDidLoad {
    ...
    //找本地EOS钱包
    NSArray *walletarr = [CreateAll GetWalletArrayByCoinType:EOS];
    if (walletarr.count > 0) {
        ...
        [self.view showMsg:NSLocalizedString(@"没有可用的EOS钱包", nil)];
    }
...
-(void)operationBtnAction:(UIButton *)btn{
    ...
    if([NSString checkEOSAccount:VALIDATE_STRING(self.eosAccount)] == NO){
        [self.view showMsg:NSLocalizedString(@"EOS账户名称为a-z与1-5组合的12位字符", nil)];
        return;
    }
        ...
        if (response) {
            ...
            if (dic) {
                ...
                if (balance > 0.2) {
                    ...
                    [weakSelf.deview.infolb setText:[NSString stringWithFormat:@"%@",NSLocalizedString(@"创建EOS帐号", nil)]];
...
-(void)initHeadView{
    ...
    _titlelabel.text = NSLocalizedString(@"创建EOS账户", nil);
