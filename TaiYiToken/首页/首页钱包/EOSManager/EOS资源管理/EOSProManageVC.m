...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"EOSProManageVC"];
}
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [MobClick endLogPageView:@"EOSProManageVC"];
}
- (void)popAction{
...
//            NSLog(@"res\n\n%@",response);
            ...
            CGFloat cpureclaiming = [info.refund_request.cpu_amount componentsSeparatedByString:@" EOS"].firstObject.doubleValue;
            CGFloat netreclaiming = [info.refund_request.net_amount componentsSeparatedByString:@" EOS"].firstObject.doubleValue;
            CGFloat reclaiming = cpureclaiming + netreclaiming;
            CGFloat total = balance + stake + reclaiming;
            weakSelf.progressView = [[JWProgressView alloc]initWithFrame:CGRectMake(ScreenWidth/2 - 40, SafeAreaTopHeight + 20, 100, 100)];
...
//            [NSTimer scheduledTimerWithTimeInterval:0.05 target:weakSelf selector:@selector(changeProgressValue) userInfo:nil repeats:YES];
            ...
            [weakSelf.pro mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.top.equalTo(SafeAreaTopHeight + 130);
            ...
            [weakSelf.detailView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.right.equalTo(0);
                make.top.equalTo(SafeAreaTopHeight + 260);
