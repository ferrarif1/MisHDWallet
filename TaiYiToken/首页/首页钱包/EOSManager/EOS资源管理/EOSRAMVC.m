...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"EOSRAMVC"];
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [MobClick endLogPageView:@"EOSRAMVC"];
}
- (void)popAction{
...
- (void)viewDidLoad {
    ...
    if (_eosAccountInfo) {
        ...
        MJWeakSelf
        [self getTableRowsSuccess:^(id response) {
            if (response != nil) {
...
-(void)textFieldDidChange :(UITextField *)textField{
    if (textField.tag == 2930) {
        ...
        if (self.ramView.buyBtn.selected == YES && self.ramView.saleBtn.selected == NO) {
            ...
            [_ramView.amountLabel setText:[NSString stringWithFormat:@"≈%.4fEOS",self.ramPrice * amount/1024.0]];
        }
    }
}
...
- (void)getTableRowsSuccess:(void(^)(id response))handler{
...
//    NSString *memo = @"";
    ...
    if (self.ramView.buyBtn.selected == YES && self.ramView.saleBtn.selected == NO) {
        ...
        NSString *buyamount = [NSString stringWithFormat:@"%.4f EOS",self.ramView.manageTextField.text.doubleValue];
