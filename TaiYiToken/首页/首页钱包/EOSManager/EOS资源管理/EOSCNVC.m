...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"EOSCNVC"];
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [MobClick endLogPageView:@"EOSCNVC"];
}
- (void)popAction{
...
//        [_gasView.namelb setText:NSLocalizedString(@"抵押比例", nil)];
        ...
        self.reclaimView.cpureclaim = [NSString stringWithFormat:@"%.4f EOS",cpu*0.0001];
        self.reclaimView.netreclaim = [NSString stringWithFormat:@"%.4f EOS",net*0.0001];
...
-(void)operationBtnAction:(UIButton *)btn{
    ...
    if (self.detailView.stakeBtn.selected == YES && self.detailView.reclaimBtn.selected == NO) {
        ...
        if ([NSString checkNumber:amount] == NO || amount == nil || [amount isEqualToString:@""]) {
            [self.view showMsg:NSLocalizedString(@"EOS数量错误", nil)];
            return;
        }
        ...
        [_deview.amountlb setText:[NSString stringWithFormat:@"%.4f EOS",cpu.doubleValue+net.doubleValue]];
...
//    抵押/赎回
    ...
    if (self.detailView.stakeBtn.selected == YES && self.detailView.reclaimBtn.selected == NO) {
        ...
        [args setObject:[NSString stringWithFormat:@"%.4f EOS",self.eosamount*self.gasView.gasSlider.value] forKey:@"stake_cpu_quantity"];
        [args setObject:[NSString stringWithFormat:@"%.4f EOS",self.eosamount*(1.0 -self.gasView.gasSlider.value)] forKey:@"stake_net_quantity"];
        [args setObject:@"0" forKey:@"transfer"];
    }else if (self.detailView.stakeBtn.selected == NO && self.detailView.reclaimBtn.selected == YES){
        [args setObject:VALIDATE_STRING(from) forKey:@"from"];
        [args setObject:VALIDATE_STRING(from) forKey:@"receiver"];
        [args setObject:[NSString stringWithFormat:@"%.4f EOS",self.reclaimView.cpuTextField.text.doubleValue] forKey:@"unstake_cpu_quantity"];
        [args setObject:[NSString stringWithFormat:@"%.4f EOS",self.reclaimView.netTextField.text.doubleValue] forKey:@"unstake_net_quantity"];
    }
